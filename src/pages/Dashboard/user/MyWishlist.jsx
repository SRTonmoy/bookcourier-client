...
export default function MyWishlist() {
  ...
  const handleMoveToCart = (bookId, bookName) => {
    window.dispatchEvent(new CustomEvent('show-toast', {
      detail: { 
        type: 'info', 
        message: `"${bookName}" added to cart (demo)` 
      }
    }));
  };
    ...
    <div className="p-6">
      ...
      {wishlistItems?.length > 0 && (
        <div className="alert alert-info mb-4">
          <AlertCircle size={20} />
          <div>
            <p className="text-sm">Debug: Found {wishlistItems.length} items. First item ID: {wishlistItems[0]?.bookId}</p>
          </div>
        </div>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {wishlistItems.map((item) => {
              ...
              const bookIdStr = item.bookId?.toString();
              
              return (
                <div 
                  key={item._id || bookIdStr} 
                  className={`card bg-base-100 shadow-lg hover:shadow-xl transition-shadow border-2 ${
                    selectedItems.has(bookIdStr) ? 'border-primary' : 'border-transparent'
                  }`}
                >
                  <div className="card-body p-5">
                    ...
                    <div className="form-control">
                      <label className="label cursor-pointer justify-start gap-3 p-0">
                        <input 
                          ...
                          checked={selectedItems.has(bookIdStr)}
                          onChange={() => toggleSelectItem(bookIdStr)}
                        />
                    ...
                    <div className="flex gap-4 mt-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <Link to={`/books/${bookIdStr}`} className="hover:no-underline">
                    ...
                    <div className="card-actions justify-between mt-4">
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleMoveToCart(bookIdStr, item.bookName)}
                      ...
                      <button
                        onClick={() => handleRemove(bookIdStr)}
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      // Find book names for selected items
                      const selectedBooks = wishlistItems
                        .filter(item => selectedItems.has(item.bookId?.toString()))
                      ...
                      if (selectedBooks.length > 0) {
                        window.dispatchEvent(new CustomEvent('show-toast', {
                          detail: { 
                            type: 'info', 
                            message: `Added ${selectedBooks.length} books to cart (demo)` 
                          }
                        }));
                      }
                    }}
